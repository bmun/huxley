...
class SecretariatMemberAdmin(admin.ModelAdmin):
    def load(self, request):
        ...
        for row in reader:
            ...
            sm = SecretariatMember(name=row[0],committee=row_committee,is_head_chair=bool(row[2]))
            sm.save()

        return HttpResponseRedirect(reverse('admin:core_secretariatmember_changelist'))
