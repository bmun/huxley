...
class SchoolTest(TestCase):
    ...
    def test_update_fees(self):
        ...
        #tests before assignments given out
        self.assertEquals(
            school.fees_owed,
        ...
        Assignment.objects.bulk_create([
            Assignment(committee_id=cm, country_id=ct, school_id=school.id, is_double=False)
            for ct in [ct1, ct2]
            for cm in [cm1, cm2]
        ])

        #test if fee is calculated using assingments 
        school.save()
        self.assertEquals(
            school.fees_owed,
            school.REGISTRATION_FEE + school.DELEGATE_FEE * 4,
        )

        Assignment.objects.bulk_create([
            Assignment(committee_id=cm, country_id=ct, school_id=school.id, is_double=True)
            for ct in [ct3]
            for cm in [cm1, cm2]
        ])

        #tests if it detects difference between single and double delegates (double should pay twice as much)
        school.save()
        self.assertEquals(
            school.fees_owed,
            school.REGISTRATION_FEE + school.DELEGATE_FEE * 8,
        )
...
class AssignmentTest(TestCase):
    ...
    def test_update_assignments(self):
        ...
        updates = [
            (cm1, ct1, s1, False, True),
            (cm1, ct2, s1, False, True),
            (cm1, ct3, s1, False, True),   # ADDED
            # (cm2, ct1, s1), # DELETED
            (cm2, ct2, s2, False, True),   # UPDATED
            (cm2, ct3, s2, False, True),   # ADDED
        ]
