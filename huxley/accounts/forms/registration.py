...
class RegistrationForm(forms.Form):
    ...
    first_name = forms.CharField(label="First Name", widget=forms.TextInput(attrs={'class':'half required'}))
    last_name  = forms.CharField(label="Last Name", widget=forms.TextInput(attrs={'class':'half required'}))
    ...
    school_location = forms.ChoiceField(label="Where is your school?", widget=forms.RadioSelect(attrs={'class':'international-check', 'name':'school_location'}), choices=School.LOCATION_OPTIONS, initial=School.LOCATION_USA)
    school_name     = forms.CharField(label="Official School Name", widget=forms.TextInput(attrs={'class':'full required'}))
    school_address  = forms.CharField(label="Address", widget=forms.TextInput(attrs={'class':'full required'}))
    school_city     = forms.CharField(label="City", widget=forms.TextInput(attrs={'class':'third required'}))
    school_state    = forms.CharField(label="State", widget=forms.TextInput(attrs={'class':'third required'}), required=False)
    school_zip      = forms.CharField(label="Zip", widget=forms.TextInput(attrs={'class':'third required zip'}), min_length=5)
    school_country  = forms.CharField(label="Country", initial='United States of America', widget=forms.TextInput(attrs={'class':'third', 'disabled': True}), required=False)
    ...
    times_attended      = forms.IntegerField(label="# of BMUN Sessions Attended", widget=forms.TextInput(attrs={'class':'third required IntegersOnly'}))
    min_delegation_size = forms.IntegerField(label="Min. Delegation Size", widget=forms.TextInput(attrs={'class':'third required IntegersOnly'}))
    max_delegation_size = forms.IntegerField(label="Max. Delegation Size", widget=forms.TextInput(attrs={'class':'third required IntegersOnly'}))

    primary_name    = forms.CharField(label="Name", widget=forms.TextInput(attrs={'class':'third required'}))
    primary_email   = forms.EmailField(label="Email", widget=forms.TextInput(attrs={'class':'third required email'}))
    primary_phone   = forms.CharField(label="Phone", widget=forms.TextInput(attrs={'class':'third required phoneNum phoneVal'}))
    secondary_name  = forms.CharField(label="Name", widget=forms.TextInput(attrs={'class': 'third'}), required=False)
    secondary_email = forms.EmailField(label="Email", widget=forms.TextInput(attrs={'class':'third email'}), required=False)
    secondary_phone = forms.CharField(label="Phone", widget=forms.TextInput(attrs={'class':'third phoneNum phoneVal'}), required=False)
