...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    country_preferences = serializers.ListField(child=serializers.IntegerField(required=False), source='country_preference_ids')
    committee_preferences = serializers.PrimaryKeyRelatedField(allow_empty=True, many=True, queryset=Committee.objects.all(), required=False)
    ...
    def validate(self, data):
        ...
        num_spanish_speaking_delegates = data.get('num_spanish_speaking_delegates')
        num_chinese_speaking_delegates = data.get('num_chinese_speaking_delegates')

        total_delegates = sum((
            num_beginner_delegates or 0,
            num_intermediate_delegates or 0,
            num_advanced_delegates or 0))

        if num_spanish_speaking_delegates > total_delegates:
            invalid_fields['num_spanish_speaking_delegates'] = 'Cannot exceed total number of delegates.'
        if num_chinese_speaking_delegates > total_delegates:
            invalid_fields['num_chinese_speaking_delegates'] = 'Cannot exceed total number of delegates.'
