...
class InCommitteeFeedbackDetailCreateTestCase(tests.CreateAPITestCase):
    ...
    def test_anonymous_user(self):
        '''Unauthenticated users shouldn't be able to create feedback.'''
        response = self.get_response(self.feedback.id)
        self.assertNotAuthenticated(response)

    def test_advisor(self):
        '''Advisors should not be able to create feedback.'''
        self.client.login(username='advisor', password='advisor')
        response = self.get_response(self.feedback.id)
        self.assertPermissionDenied(response)
    ...
    def test_delegate(self):
        '''Delegates should not be able to create feedback'''
        self.client.login(username='delegate', password='delegate')
        response = self.get_response(self.feedback.id)
        self.assertPermissionDenied(response)

    def test_superuser(self):
        '''Should return correct data; superusers should be able to create feedback'''
        superuser = models.new_superuser(username='s_user', password='s_user')
        self.client.login(username='s_user', password='s_user')
        response = self.get_response(self.feedback.id)
        self.assertEqual(response.data, {
