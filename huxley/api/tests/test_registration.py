...
class RegistrationListPostTest(CreateAPITestCase):
	...
	def test_valid(self):
		...
		registration_query = Registration.objects.filter(id=response.data['id'])
	...
	def test_empty_fields(self):
		...
		params = self.get_params(num_beginner_delegates='',
		...
		self.assertEqual(response.data,
			{'num_beginner_delegates': [u'A valid integer is required.'],
			 ...
			 'num_chinese_speaking_delegates': [u'A valid integer is required.'],
			 'num_intermediate_delegates': [u'A valid integer is required.'],
			 'num_spanish_speaking_delegates': [u'A valid integer is required.'],
	...
	def test_fees(self):
		...
		school = models.new_school()
		conference = Conference.get_current()
		params = self.get_params(school=school.id, conference=conference.session, fees_owed=2000.0, fees_paid=2000.0)
		response = self.get_response(params=params)
	...
	def test_country_preferences(self):
		...
		school = models.new_school()
		conference = Conference.get_current()
		params = self.get_params(school=school.id, conference=conference.session, countrypreferences=[0, c1, c2, 0, c1])
		response = self.get_response(params=params)
	...
	def test_delegate_total(self):
		...
		response = self.get_response(params=params)
		self.assertEqual(response.data,
			{'num_spanish_speaking_delegates': [u'Cannot exceed total number of delegates.'],
			 'num_chinese_speaking_delegates': [u'Cannot exceed total number of delegates.']})
